...
from models.PPO import PPOAgentVector
...
def main():
    ...
    # 환경 초기화
    LOOKAHEAD_POINTS = 5
    env = MoraiEnv(action_bounds=ACTION_BOUNDS, csv_path=CENTERLINE_CSV_PATH, lookahead=LOOKAHEAD_POINTS)
    sensor = env.sensor
    ...
    obs_dim = 3 + 2*LOOKAHEAD_POINTS
    
    # 에이전트 생성
    agent = PPOAgentVector(obs_dim, 2, ACTION_BOUNDS, log_dir=LOG_DIR)
    # 이전 모델 불러오기
