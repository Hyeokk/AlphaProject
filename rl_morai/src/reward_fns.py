...
class RewardFns:
    @staticmethod
    def path_following_reward(lookahead=5, target_speed=20.0,
                              w_lat=1.0, w_speed=0.5):
        def reward_fn(obs):
            # obs = [x, y, v, dx0, dy0, ..., dxK, dyK]
            v = obs[2]
            future = np.array(obs[3:]).reshape(-1,2)
            dists = np.linalg.norm(future, axis=1)
            # 가까운 점에 더 높은 가중치
            weights = np.linspace(1.0, 0.4, len(dists))
            lateral_cost = np.average(dists, weights=weights)

            speed_reward = 1 - abs(v-target_speed)/target_speed
            speed_reward = max(speed_reward, 0.0)

            return -(w_lat*lateral_cost) + w_speed*speed_reward
        return reward_fn
